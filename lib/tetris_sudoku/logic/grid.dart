...
class Grid {
  ...
  Grid() : _grid = List<GridState>.filled(Settings.gridSize * Settings.gridSize, GridState.CLEAR);
  ...
  void set(Piece piece, int x, int y, [GridState value = GridState.SET]) {
    ...
    setValues(piece.occupations, position.x, position.y, value);
  }

  void setValues(List<List<bool>> occupations, int x, int y, [GridState value = GridState.SET]) {
    int currY = y;
    for (var row in occupations) {
      int currX = x;
      for (var element in row) {
        setValue(currX, currY, element || _grid[currX + Settings.gridSize * currY] != GridState.CLEAR ? value : GridState.CLEAR);
        currX++;
      }
      currY++;
    }
  ...
  bool _doesFit(List<List<bool>> occupations, int x, int y) {
    ...
    for (var row in occupations) {
      int currX = x;
      for (var element in row) {
        if (notInGrid(currX, currY) || (isSet(currX, currY) && element == true)) {
          return false;
        }
        currX++;
      }
    ...
    for (int offY = -2; offY < sizeY + 2; offY++)
      for (int offX = -2; offX < sizeX + 2; offX++) {
        if (_doesFit(occupations, x + offX, y + offY)) {
          ...
          if (best == null || center.squaredDistanceTo(best) > center.squaredDistanceTo(current)) {
            best = current;
